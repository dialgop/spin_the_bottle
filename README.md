...
- **`nao_webots_driver`** (done) — no simulated camera; NAO's "vision" is `vision_standalone`'s recorded footage (see above), not a Webots camera feed, which keeps the simulation light. `HeadYaw`/`HeadPitch` and both shoulders (`LShoulderPitch`/`LShoulderRoll`, `RShoulderPitch`/`RShoulderRoll`) are driven through `ros2_control` (`joint_trajectory_controller`s: `head_controller`, `arm_controller`), confirmed working end-to-end against a `nao_ros2.wbt` world (adapted from Webots' `nao_demo`, with NAO's controller set to `<extern>`). NAO's head and shoulder joints are modeled in Webots' `Nao.proto` as `Hinge2Joint` (2 degrees of freedom in one joint), which Webots can't auto-export to URDF, so `resource/nao_webots.urdf` is a hand-written joint chain (using NAO's real joint limits) published via a plain `robot_state_publisher` node, rather than relying on Webots' auto-export.
- **`nao_vision_referee`** (done) — a `referee_node` that compiles `vision_standalone`'s `pre_game`/`line_projection`/`world_coordinates`/`face_detection` sources directly (no duplication). It finds the frame where a recorded bottle-spin video settles, publishes the resulting head pose to `head_controller`, raises whichever arm is on that side (a hand-written pointing gesture, not ported from `vision_standalone` - this is robot-arm kinematics, not vision) to `arm_controller`, then checks a recorded face video for a face and logs the referee's decision ("ask them to spin next" / "ask the group to spin again") — confirmed working end-to-end, including moving the simulated NAO's head and arm to match the video's pointing angle. The face check reads a recorded video rather than a live/simulated camera frame, same as everything else here — it's there to test the ROS 2 wiring, not to be the final input source.
...
2. Bend the elbow as part of the pointing gesture, not just the shoulder.
