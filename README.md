...
- **`nao_vision_referee`** (done) — a `referee_node` that compiles `vision_standalone`'s `pre_game`/`line_projection`/`world_coordinates`/`face_detection` sources directly (no duplication). It finds the frame where a recorded bottle-spin video settles, publishes the resulting head pose to `head_controller`, raises whichever arm is on that side (shoulder up, elbow straightened) to `arm_controller` while the other rests at a natural bend, then checks a recorded face video for a face and logs the referee's decision ("ask them to spin next" / "ask the group to spin again") — confirmed working end-to-end, including moving the simulated NAO's head and arm to match the video's pointing angle. The arm-pointing gesture is hand-written, not ported from `vision_standalone` - it's robot-arm kinematics, not vision. The face check reads a recorded video rather than a live/simulated camera frame, same as everything else here — it's there to test the ROS 2 wiring, not to be the final input source.
